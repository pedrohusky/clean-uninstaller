...
class Registry:
    ...
    def edit_context_menu_item(self, name, command):
        try:
            ...
            command_key = winreg.OpenKey(
                winreg.HKEY_CLASSES_ROOT, key_name, 0, winreg.KEY_WRITE
            )
            ...
            command_key = winreg.OpenKey(
                winreg.HKEY_CLASSES_ROOT, key_name + "\\command", 0, winreg.KEY_WRITE
            )
    ...
    def backup_registry():
        ...
        backup_path = os.path.join(
            os.environ["ProgramFiles"], "Desinstalar Tudo", "registry_backup.reg"
        )
        try:
        ...
        for sub in sub_keys:
            try:
                ...
                for _ in range(num):
                    ...
                    if should_delete:
                        print(
                            f"Deleting registry key: {self.hkey_names[root]}\\{sub}\\{child}"
                        )
    ...
    def find_and_remove_registry_entries(self, read_only=False, registry_files=None):
        try:
            ...
            if registry_files:
                ...
                registry_files_sorted = sorted(
                    registry_files, key=lambda path: path.count("\\"), reverse=True
                )
                ...
                for registry_path in registry_files_sorted:
                    ...
                    if root_key and subkey:
                        print(
                            f"Searching for registry keys: {self.hkey_names[root_key]}\\{subkey}"
                        )
                        registry_keys.extend(
                            self.delete_sub_key(root_key, [subkey], should_delete=True)
                        )
                ...
                for root in self.registry_paths:
                    ...
                    sub_keys_set.add(
                        root[1] + f"\\{self.uninstaller.folder_name.replace(' ', '')}"
                    )
                    sub_keys_set.add(root[1] + f"\\{self.uninstaller.program_name}")
                    sub_keys_set.add(
                        root[1] + f"\\{self.uninstaller.program_name.replace(' ', '')}"
                    )
                    ...
                    registry_keys.extend(
                        self.delete_sub_key(
                            root[0], sub_keys, should_delete=not read_only
                        )
                    )
